...
static void ConvertDataFile(Home_t *home, char *baseDataFileName)
{
        ...
        int  nextSeqNum, notDone, conversionNeeded;
        int  binRead = 0;
...
/*
 *          If we have an open data file segment, read the next line
 */
            if (fpOld != (FILE *)NULL) {
...
/*
 *                  If there are no changes needed to the nodal data
 *                  just dump it back out to the new file.  (Note: the
 *                  linefeed was stripped off by Getline(), so we
 *                  need to add that back on)
 */
                    fprintf(fpNew, "%s", inLine);

                } else {
/*
 *                  This is just a place holder since there are no
 *                  nodal data conversions needed yet for existing
 *                  versions of the data file...  If a conversion
 *                  becomes necessary in the future, process all
 *                  data for a single node in this block.
 */
                }
