...
{
        int  i, j;
        char *argName, *argValue, *token, *suffix;
 ...
 *
 *------------------------------------------------------------------------*/
static void GetRepartULoadData(int numCells, CellData_t *cellList,
                               real8 **uloadData)
{
        int   cellIndex;
        real8 *cellLoadData;
...
static void GetDecompCnt(int xNumDoms, int yNumDoms, int zNumDoms,
                         int *totDecomps)
{
        int  i;
 ...
 */
        for (i = 0; i < 8; i++) {
            ...
            switch (i) {
                case LLF:
                    GetDecompCnt(xDoms[0], yDoms[0], zDoms[0], totDecomps);
                    break;
                case LRF:
                    GetDecompCnt(xDoms[1], yDoms[0], zDoms[0], totDecomps);
                    break;
                case ULF:
                    GetDecompCnt(xDoms[0], yDoms[1], zDoms[0], totDecomps);
                    break;
                case URF:
                    GetDecompCnt(xDoms[1], yDoms[1], zDoms[0], totDecomps);
                    break;
                case LLB:
                    GetDecompCnt(xDoms[0], yDoms[0], zDoms[1], totDecomps);
                    break;
                case LRB:
                    GetDecompCnt(xDoms[1], yDoms[0], zDoms[1], totDecomps);
                    break;
                case ULB:
                    GetDecompCnt(xDoms[0], yDoms[1], zDoms[1], totDecomps);
                    break;
                case URB:
                    GetDecompCnt(xDoms[1], yDoms[1], zDoms[1], totDecomps);
                    break;
            }
        }
        return;
                              ...
                              real8 minSideRatio)
{
        ...
        int slabMinIndex, slabMaxIndex;
        int xMin, yMin, zMin;
        int xMax, yMax, zMax;
        int index[3]; 
        real8 totalLoad, neededLoad, overlapFactor;
        real8 *slabLoad, *boundaries;
        real8 *xOverlap, *yOverlap, *zOverlap;
