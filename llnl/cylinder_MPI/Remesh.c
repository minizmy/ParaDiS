 ...
 */
#ifdef  _CYLINDER //(iryu/2011.11.30)
 ...
 */
#ifdef  _CYLINDER //(iryu/2011.11.30)
                if (nbr->constraint != CYLINDER_SURFACE_NODE) {
                    j++;
                    continue;
                }
#else
 ...
 *-----------------------------------------------------------------------*/
#ifdef  _CYLINDER
void Remesh(Home_t *home, Cylinder_t *cylinder)
#else
...
#endif
{
        ...
        if (param->remeshRule < 0) 
	{
		printf("Turning off the Remesh!.\n");
		return;
	}
...
#ifdef MPIDEBUGMODE
        if(home->myDomain == 0) printf("Remesh: RemeshRule finished \n");
#endif
#ifdef _FEM
/*
...
#ifdef _CYLINDER
	Cylinder_Remesh(home,cylinder);
#endif
#ifdef MPIDEBUGMODE
    printf("Remesh: %d  Cylinder_Remesh finished \n", home->myDomain);
#endif
/*
...
#ifdef MPIDEBUGMODE
        if(home->myDomain == 0) printf("Remesh: CommSendRemesh finished \n");
#endif
        TimerStart(home, FIX_REMESH);
        FixRemesh(home);
        TimerStop(home, FIX_REMESH);
#ifdef MPIDEBUGMODE
        if(home->myDomain == 0) printf("Remesh: FixRemesh finished \n");
#endif
/*
...
#ifdef MPIDEBUGMODE
        if(home->myDomain == 0) printf("Remesh: RemoveOrphanceNodes finished \n");
#endif
#ifdef PARALLEL
#ifdef SYNC_TIMERS
