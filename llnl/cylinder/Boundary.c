 ...
 *                    2) Be pinned	 - call Pinning
 *                    3) Be annihilated  - call Annihilate
 *
 *      (11/19/2015:iryu)              
 ...
 *         - Algorithm:
 *            1. Project all nodes which have just passed the boundary on the the boundary plane. 
 *               -  if the neighboring nodes are also on the boundary 
 *                    change the slip plane to the plane of the boundary. 
 *            2. If dislocation segments are located on the boundary
 *               - Change the slip plane according to the Energy criterion
 *
 *      Included functions:
 *              AddNodeatTB()
 *              Pinning()
...
#ifdef  _CYLINDER 
void Boundary(Home_t *home, Cylinder_t *cylinder)
#else   
void Boundary(Home_t *home)
#endif 
{
	...
	// Boudnary  : Tx*x + Ty*y + Tz*z + Td = 0
	// Tx = 0.0;	Ty = 1.0;	Tz = 0.0;	Td = 0.0;
	Tx = 0.0;	Ty = 0.0;	Tz = 1.0;	Td = 0.0;
	TB[0] = Tx;	TB[1] = Ty; 	TB[2] = Tz; 	TB[3] = 200; 
...
//				printf("nconstraint= %f, nlc = %d\n",nconstraint,nlc); 
		   ...
		   // Find new slip plane(NTB), new Burgers(BTB) vector for node A
			   ...
			   #ifdef _CYLINDER
			   SlipConstraint1(home,cylinder,nodea,N1,TB);
			   #else
			   SlipConstraint1(home,nodea,N1,TB);
			   #endif
		   }
		   else if (nlc == 2){
		   // Project to the intersection of two slip planes. 
			   ...
			   //Get new position(CoordAnew)
			   #ifdef _CYLINDER
			   SlipConstraint2(home,cylinder,nodea,N1,N2,TB);
			   #else
			   SlipConstraint2(home,nodea,N1,N2,TB);
			   #endif
		   }
		   else{
