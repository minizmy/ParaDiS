 ...
 * 	e.g) D=1000nm NumNucSiteD = 1000 */
    ...
    for (l = NumNucSiteD; l < NumNucSiteAllD ; l++){
        ...
        cylinder->NucSite_x[l] = x1;
        cylinder->NucSite_y[l] = y1;
        cylinder->NucSite_z[l] = z1;

        do{ // Stress concentration factor is set to larger than zero and less than 3.0
 ...
 * 	e.g) D=1000nm NumNucSiteD = 1000 */
    ...
    real8  SS1, Q[NumNucSiteAllD], dtt;
...
----------------- Only for the nucleatoin test -----------*/
    ...
    for (i = 0; i < NumNucSiteAllD ; i++){		// Find the nucleation site
...
void LOOPGENERATE_FCC(Home_t *home, Cylinder_t *cylinder)
{
	...
	for (l = 0; l < NumNucSiteD ; l++){
        ...
        if (cylinder->NucSite_F[l]){ // Find the nucleation site
			...
			/* Need to make sure that a nucleated dislocation loop would expand outward from the loop center */
...
*/
			...
			Rdir = (LoopNode[1]->x-cx)*fx +(LoopNode[1]->y-cy)*fy;
			if (Rdir <= 0.0){ 
