 ...
 */
        ...
        BindVar(CPList, "gamma", &param->gamma, V_DBL, 1, VFLAG_NULL);
        param->gamma = 1.60506e-11;
#endif
 ...
 */
#ifdef _CYLINDER
        BindVar(CPList, "MobSurfaceScale", &param->MobSurfaceScale, V_DBL, 1, VFLAG_NULL);
        param->MobSurfaceScale = 1.0;
#endif
...
#ifdef _CYLINDER
	// Add specific parameters for cylinder module
	BindVar(CPList, "CYL_radius", &param->cyl_radius, V_DBL, 1, VFLAG_NULL);
        param->cyl_radius = 1;

	BindVar(CPList, "CYL_nq", &param->cyl_nq, V_INT, 1, VFLAG_NULL);
        param->cyl_nq = 21;
	BindVar(CPList, "CYL_nz", &param->cyl_nz, V_INT, 1, VFLAG_NULL);
        param->cyl_nz = 21;

	BindVar(CPList, "NimgPBC", &param->NimgPBC, V_INT, 1, VFLAG_NULL);
        param->NimgPBC = 1;

	BindVar(CPList, "Loading_Direction", &param->Loading_Direction, V_INT, 1, VFLAG_NULL);
        param->Loading_Direction = 1;

        BindVar(CPList, "CYL_VSLength", &param->cyl_VSlength, V_DBL, 1,
                VFLAG_NULL);
        param->cyl_VSlength = 1e4;
// (2012.12.20/iryu)
// For using nonlinear mobility law 
        BindVar(CPList, "threshold", &param->threshold, V_DBL, 1, VFLAG_NULL);
        param->threshold = 0.0;
// (2013.10.22/iryu)
// For anisotropic loading 
        BindVar(CPList, "ANISO_110", &param->ANISO_110, V_DBL, 1, VFLAG_NULL);
        param->ANISO_110 = 0;
        BindVar(CPList, "ANISO_111", &param->ANISO_111, V_DBL, 1, VFLAG_NULL);
        param->ANISO_111 = 0;
#ifdef _TORSION
        BindVar(CPList, "AppliedTheta", &param->AppliedTheta, V_DBL, 1, VFLAG_NULL);
        param->AppliedTheta = 0.0;

        BindVar(CPList, "ThetaRate", &param->ThetaRate, V_DBL, 1, VFLAG_NULL);
        param->ThetaRate = 0.0;

        BindVar(CPList, "pTheta", &param->pTheta, V_DBL, 1, VFLAG_NULL);
        param->pTheta = 0.0;

        BindVar(CPList, "dot_dpTheta_tol", &param->dot_dpTheta_tol, V_DBL, 1, VFLAG_NULL);
        param->dot_dpTheta_tol = 1.e3;

        BindVar(CPList, "dot_appliedTheta", &param->dot_appliedTheta, V_DBL, 1, VFLAG_NULL);
        param->dot_appliedTheta = 1.e-6;

        BindVar(CPList, "Zboundary", &param->Zboundary, V_INT, 1, VFLAG_NULL);
        param->Zboundary = 0;

#endif

// (2013.05.13/iryu)
// Dislocation nucleation : Loop radius 
#ifdef _NUCLEATION
        BindVar(CPList, "LoopRadius", &param->LoopRadius, V_DBL, 1, VFLAG_NULL);
        param->LoopRadius = 20.0;

        BindVar(CPList, "NucTEMP", &param->NucTEMP, V_DBL, 1, VFLAG_NULL);
        param->NucTEMP = 298.15;

        BindVar(CPList, "NucTEMPm", &param->NucTEMPm, V_DBL, 1, VFLAG_NULL);
        param->NucTEMPm = 700.0;

#ifdef _TORSION
        BindVar(CPList, "NucSTRESSCon", &param->NucSTRESSCon, V_DBL, 1, VFLAG_NULL);
        param->NucSTRESSCon = 1.0;
        BindVar(CPList, "NucSTRESSConGB", &param->NucSTRESSConGB, V_DBL, 1, VFLAG_NULL);
        param->NucSTRESSConGB = 1.2;
#else
        BindVar(CPList, "NucSTRESSCon", &param->NucSTRESSCon, V_DBL, 1, VFLAG_NULL);
        param->NucSTRESSCon = 2.0;
        BindVar(CPList, "NucSTRESSConGB", &param->NucSTRESSConGB, V_DBL, 1, VFLAG_NULL);
        param->NucSTRESSConGB = 2.4;
#endif
...
#if defined _BOUNDARY /*iryu*/
	BindVar(CPList, "NumTB", &param->NumTB, V_DBL, 1, VFLAG_NULL);
	param->NumTB = 0;

	//  Internal Boundary Plane
	//  : IntBoundary[0]*x + IntBoundary[1]*y + IntBoundary[2]*z + IntBoundary[3] = 0 

        BindVar(CPList, "IntBoundary",param->IntBoundary, V_DBL, 4, VFLAG_NULL);
        param->IntBoundary[0] = 0.0;
        param->IntBoundary[1] = 1.0;
        param->IntBoundary[2] = 0.0;
        param->IntBoundary[3] = 0.0;
#endif 
