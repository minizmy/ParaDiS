...
#ifdef _BOUNDARY
/* (iryu/2015.12.13)
   INTERNAL_BOUNDARY_NODE should move on the internal bouandry plane 
    - Project velocity vector on the internal boundary plane
    - Vnew = Vold - dot(Vold,T)*T
     where, T is the internal boundary plane normal
 */

	if (node->constraint == INTERNAL_BOUNDARY_NODE)
	{
	    real8 Tx, Ty, Tz, Td, normT;
	    real8 Vx, Vy, Vz, normV;
	    real8 Vxnew, Vynew, Vznew;
	    real8 tol2 = 0.0001;
	    ...
	    Tx/=normT;	Ty/=normT;	Tz/=normT;	Td/=normT;
//	    Tx = 0.0;	Ty = 1.0;	Tz = 0.0;	Td = 0.0;
	    ...
	    Vxnew = Vx - Tx*(Tx*Vx + Ty*Vy + Tz*Vz);
	    Vynew = Vy - Ty*(Tx*Vx + Ty*Vy + Tz*Vz);
	    Vznew = Vz - Tz*(Tx*Vx + Ty*Vy + Tz*Vz);

	    // check if Vnew is normal to T
	    if (fabs(Vxnew*Tx + Vynew*Ty + Vznew*Tz) > tol2*normV)
	    {
		printf("Mobility : at INTERNAL_BOUNDARY_NODE (%d,%d) \n",
			node->myTag.domainID, node->myTag.index);
		printf("T = [%f,%f,%f];  Vold = [%f,%f,%f];  Vnew = [%f,%f,%f];\n",
			Tx, Ty, Tz, Vx,Vy,Vz, Vxnew,Vynew,Vznew);
		printf("dot(Vnew,T)=%f; tol=%f;\n",
			fabs(Vxnew*Tx + Vynew*Ty + Vznew*Tz), tol2);
		Fatal("Vnew should be on the internal boundary plane \n");
	    }
