...
//#ifdef _BOUNDARY  // (iryu/12.30,2015)
//		  if ((nodeb->constraint == UNCONSTRAINED) || (nodeb->constraint == INTERNAL_BOUNDARY_NODE))
//#else
...
//#endif
  ...
  while (dirty) 
      ...
      for (i = 0; i < home->newNodeKeyPtr; i++) 
	{
	  ...
	  if (nodea->constraint == PINNED_NODE) continue;
//#ifdef _BOUNDARY  // (iryu/12.30,2015)
//	  if ((nodea->constraint != UNCONSTRAINED) && (nodea->constraint != INTERNAL_BOUNDARY_NODE)) continue;
//#else
	  if (nodea->constraint != UNCONSTRAINED) continue;
//#endif
//#ifdef _BOUNDARY  // (iryu/12.30,2015)
//	  if ((nodea->constraint == UNCONSTRAINED) || (nodea->constraint == INTERNAL_BOUNDARY_NODE))
//#else
