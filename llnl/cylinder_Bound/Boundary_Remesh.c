...
#ifdef _CYLINDER 
void Boundary_Remesh(Home_t *home, Cylinder_t *cylinder)
#else   
void Boundary_Remesh(Home_t *home)
#endif 
{
 ...
 */
	...
	// Change the node flag to "INTERNAL_BOUNDARY_NODE"
        if (TBpassA <= 0.0){
...
/* Step 2 :
 * Adjust coordinates of all INTERNAL_BOUNDARY_NODEs onto the internal boundary plane
 */
    if (iprint){
        printf("*** Step 2:\n");
 ...
 */
	...
	for (i = 0; i < home->newNodeKeyPtr; i++){
	    ...
	    // if (distanct between X0 and X1) > cylidner length continue
	    if (sqrt((x1-x0)*(x1-x0) + (y1-y0)*(y1-y0) + (z1-z0)*(z1-z0)) > R) continue;

	    // If a CYLINDER_SURFACE_NODE stay on the internal boundary plane, 
	    // no action is required. 
	    OnTheBoundaryA = (Tx*x1 + Ty*y1 + Tz*z1 + Td);
	    if (fabs(OnTheBoundaryA)<tol){
		if (iprint){
		    printf(" CYLINDER_SURFACE_NODE(%d,%d) move on the internal boundary\n", 
			nodea->myTag.domainID, nodea->myTag.index);
		}
		continue;
	    }
	    ...
	    // move it both on the internal boundary plane and cylinder surface
	    TBpassA = (Tx*x0+Ty*y0+Tz*z0+Td)*(Tx*x1+Ty*y1+Tz*z1+Td);
	    if (TBpassA <= 0.0){
		if (iprint){
		    printf(" CYLINDER_SURFACE_NODE (%d,%d) passes the internal boundary plane ***\n", 
			nodea->myTag.domainID, nodea->myTag.index);
		}
		// *** Step 3-1 : Find (x2,y2,z2) on the internal boundary plane ***
		// (x2,y2,z2) is located at the intersection of the internal boundary plane and 
		// the line which passes through the old position and new position
		
		alpha1 = (Td + Tx*x0 + Ty*y0 + Tz*z0)/(Tx*(x0 - x1) + Ty*(y0 - y1) + Tz*(z0 - z1));
		x2 = x0 + alpha1*(x1-x0);
		y2 = y0 + alpha1*(y1-y0);
		z2 = z0 + alpha1*(z1-z0);
		
		OnTheBoundaryA = (Tx*x2 + Ty*y2 + Tz*z2 + Td);
		if (iprint){
		    printf("X0 = [%f,%f,%f];\n", x0, y0, z0);
		    printf("X1 = [%f,%f,%f];\n", x1, y1, z1);
		    printf("X2 = [%f,%f,%f]; OnTheBoundary=%f \n", x2, y2, z2,OnTheBoundaryA);
		}
		if (fabs(OnTheBoundaryA)>tol){
		    Fatal("Fail to move X2 on the internal boundary : OnTheBoundaryA= %f \n",
				    OnTheBoundaryA);
		}
		// *** Step 3-2 : Find (xnew,ynew,znew) on the cylinder surface **
		//
		...
		for (j = 0; j < nodea->numNbrs; j++){
		    ...
		    // only consider the slip constraint which motion of (X1-X0) is satisfied with 
		    if (fabs(Nx*(x1-x0)+Ny*(y1-y0)+Nz*(z1-z0))>tol) continue;
		...
		if (nlc==0){
		// Xnew is obtained by projecting(x2,y2,z2) along P(px,py,pz) on the cylinder surface, 
		// where P is the vector from the center point on the same slip plane to the Xnew
		    if (fabs(Tz)>tol)
		    {
			beta = -1.0*Td/Tz;
		    ...
		    px = x2;	py = y2;    pz = z2-beta;
		    normP = sqrt(px*px + py*py + pz*pz);
		    px/=normP;	py/=normP; pz/=normP;  //normalize it
		    
  		    alphaT0 = R*R*px*px + R*R*py*py - px*px*y2*y2 + 2.0*px*py*x2*y2 - py*py*x2*x2;
		    alphaT1 = -(px*x2 + py*y2 + sqrt(alphaT0))/(px*px + py*py);
		    alphaT2 = -(px*x2 + py*y2 - sqrt(alphaT0))/(px*px + py*py);
		    if (fabs(alphaT1) > fabs(alphaT2)){
			alpha2 = alphaT2;
		    }else {
			alpha2 = alphaT1;
		    }
			...
			printf("nlc=0 \n");
			printf("P = [%f,%f,%f] \n", px, py, pz);
			printf("beta = %f; alpha2 = %f \n", beta, alpha2);
			printf("Xnew = [%f,%f,%f] \n", nodea->x, nodea->y, nodea->z);
		    }
		}else if(nlc>=1){
		// Xnew is obtained by projecting(x2,y2,z2) along P(px,py,pz) on the cylinder surface, 
		// where P is the intersection line of N(slip plane) & T(normal to internal boundary plane)
			...
			alphaT2 = -(px*x2 + py*y2 - sqrt(alphaT0))/(px*px + py*py);
			 ...
			 // X2 is outside the circle and 
			 // the projected position cannot be on the cylinder surface
			 // In this case, X2 project along the vector from the neighbor node
			if ((sqrt(nbr1x*nbr1x + nbr1y*nbr1y) < R)&&
			    (nbr1Constraint == INTERNAL_BOUNDARY_NODE))	{
			    if (iprint){
				printf("nbr is INTERNAL_BOUNDARY_NODE, P = X2-nbr1\n");
			    }
			    px = x2-nbr1x;
			    py = y2-nbr1y;
			    pz = z2-nbr1z;
		    ...
		    znew = z2 + alpha2*pz;
		    nodea->x = xnew;	nodea->y = ynew;	nodea->z = znew;
		    // nodea->constraint = PINNED_NODE;  // Maynot need change the flag(iryu/12.31.2015)
		    if (iprint){	
//			printf("nlc=1, change to PINNED_NODE(constraint = %d)\n", nodea->constraint);
			printf("nlc=1 \n");
			printf("N = [%f,%f,%f] \n", Nx1,Ny1,Nz1);
			printf("T = [%f,%f,%f] \n", Tx, Ty, Tz);
			printf("P =cross(N,T) = [%f,%f,%f] \n", px, py, pz);
			printf("alpha2 = %f \n", alpha2);
			printf("Xnew = [%f,%f,%f] \n", nodea->x, nodea->y, nodea->z);
		    }
		}

		// Check if CYLINDER_SURFACE_NODE which has just passed the internal boundary plane is moved 
		// both onto the internal boundary plane and cylinder surface. 
		OnTheBoundaryA = (Tx*xnew + Ty*ynew + Tz*znew + Td);
		OnTheCylinderSurface = (xnew*xnew + ynew*ynew - R*R);
		if (fabs(OnTheBoundaryA)>tol){
		    Fatal("Fail Xnew OnTheBoundaryA= %f\n",OnTheBoundaryA);
		}
		if (alpha2){
		    if(fabs(OnTheCylinderSurface)>tol){
			Fatal("Fail Xnew OnTheCylinderSurface= %f\n",OnTheCylinderSurface);
		    }
		}
	    }
	}
