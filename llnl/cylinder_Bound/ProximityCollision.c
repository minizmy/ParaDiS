...
/*
#ifdef _BOUNDARY
 //     If a node is a INTENRAL_BOUNDARY_NODE on the internal boundary plane, 
 //      so use that node's coordinates as the collision point
       if (node1->constraint == INTERNAL_BOUNDARY_NODE) {
            *x = p1[0];
            *y = p1[1];
            *z = p1[2];
            return;
        } else if (node2->constraint == INTERNAL_BOUNDARY_NODE){
            *x = p2[0];
            *y = p2[1];
            *z = p2[2];
            return;
