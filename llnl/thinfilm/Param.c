...
#ifdef _THINFILM 
	// Add specific parameters for THINFILM module (Sylvie Aubry)
        BindVar(CPList, "TF_halfthickness", &param->tf_halfthickness, V_DBL, 1, VFLAG_NULL);
        param->tf_halfthickness = 0.0;

        BindVar(CPList, "TF_nx", &param->tf_nx, V_INT, 1, VFLAG_NULL);
        param->tf_nx = 21;

        BindVar(CPList, "TF_ny", &param->tf_ny, V_INT, 1, VFLAG_NULL);
        param->tf_ny = 21;

        BindVar(CPList, "TF_VSLength", &param->tf_VSlength, V_DBL, 1,
                VFLAG_NULL);
        param->tf_VSlength = 1e4;

        BindVar(CPList, "TF_Mx", &param->tf_Mx, V_DBL, 1,
                VFLAG_NULL);
        param->tf_Mx = 0.0;

        BindVar(CPList, "TF_Bend_theta", &param->tf_Bend_theta, V_DBL, 1,
                VFLAG_NULL);
        param->tf_Bend_theta = 0.0;
