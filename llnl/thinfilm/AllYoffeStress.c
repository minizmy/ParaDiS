...
void YesListAllYoffeStress(Home_t *home, ThinFilm_t *thinfilm, 
 ...
 */
  ...
  segList = thinfilm->surfaceSegList;
 ...
 */
  ...
  for (i = 0; i < thinfilm->surfaceSegCount; i++) 
    {
 ...
 */
      icheck = SanityCheckYoffe(rs[2], THINFILM_SURFACE_NODE,
				rm[2], 0,t);
      if (icheck) {
         printf("Surface Node position is x=%f,y=%f,z=%f\n and surfaceZ=%f\n",rs[0],rs[1],rs[2],t);
         continue;
      }
...
void NoListAllYoffeStress(Home_t *home, ThinFilm_t *thinfilm, 
  ...
  for (i = 0; i < home->newNodeKeyPtr; i++) 
    {
      ...
      /* Node B has flag == THINFILM_SURFACE_NODE */
		     ...
		     real8 rmZ, int cstMid, real8 t)
{
  ...
  if (cstSurf !=6) 
    {
      if (iprint) printf("A node with z=%f is not flagged THINFILM_SURFACE_NODE\n",rsZ);
      YoffeStatus = 1;
    }
		    ...
		    real8 LenVirtualSeg, real8 t,real8 sigmaYoffe[3][3])
{
  ...
  // Assumes p1 has constraint = THINFILM_SURFACE_NODE and p2 has constraint = UNCONSTRAINED.
 ...
 *      to start with.
 */
        if (thinfilm->surfaceSegList != (real8 *)NULL) {
            free(thinfilm->surfaceSegList);
            thinfilm->surfaceSegList = (real8 *)NULL;
            thinfilm->surfaceSegCount = 0;
        }
 ...
 */
	...
	for (i = 0; i < home->numDomains+1; i++)
	  globalCounts[i] = localSegCounts[i];
        ...
        thinfilm->surfaceSegCount = globalCounts[home->numDomains];
 ...
 */
        if (thinfilm->surfaceSegCount > 0) {
            bufSize = thinfilm->surfaceSegCount * VALS_PER_SEG * sizeof(real8);
            thinfilm->surfaceSegList = (real8 *)malloc(bufSize);
            tmpSegCount = 0;
            ...
            MPI_Allgatherv(localSegList, numVals, MPI_DOUBLE,
                           thinfilm->surfaceSegList, globalCounts,
                           displacement, MPI_DOUBLE, MPI_COMM_WORLD);
#else
	    memcpy(thinfilm->surfaceSegList,localSegList,numVals*sizeof(double));
#endif
 ...
 */
                ...
                if (node->constraint ==THINFILM_SURFACE_NODE && nbr->constraint == THINFILM_SURFACE_NODE) continue;
 ...
 */
        if (thinfilm->surfaceSegCount > 0) {
            ...
            MPI_Allgatherv(localSegList, numVals, MPI_DOUBLE,
                           ...
                           displacement, MPI_DOUBLE, MPI_COMM_WORLD);
#else
            memcpy(thinfilm->surfaceSegList,localSegList,numVals*sizeof(real8));
#endif
