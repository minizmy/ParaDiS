 ...
 *                      collision with the thinfilm surface
 *                      
 *****************************************************************************/
...
- Flag all nodes that are outside the TF to 6.

- Split segments with a node inside (flag 0) and a node outside the TF
...
When the TF Remesh routine is finished, nodes that are flagged 6 are
...
(1) A node flagged 6 inside the TF with multiple arms. If this happens and
...
move inside the TF during a collision.
 ...
 *      Description: remove segments connecting constrained node outside the TF
 *
 *------------------------------------------------------------------------*/
static void RemoveSurfaceSegments67(Home_t *home,real8 t)
{
  ...
  while (dirty) 
      ...
      for (i = 0; i < home->newNodeKeyPtr; i++) 
	{
	  ...
	  /*
	   *   Node A is constrained to either 6 or 7 and is outside the thinfilm
	   */
	  ...
	  if (  (constra == 7 && nodea->numNbrs == 1)||  (constra == 7 && fabs(nodea->z) >=t) || (constra == 6) )
	      ...
	      for (j = 0; j < nodea->numNbrs; j++) 
		{
		  ...
		  /*
		   *   Node B is constrained to either 6 or 7 and is outside the thinfilm
		   */
 ...
 *      Description: Handle node collision with thinfilm surface
 *
 *------------------------------------------------------------------------*/
void ThinFilm_Remesh(Home_t *home,ThinFilm_t *thinfilm)
{
  ...
  // Flag to 6 nodes that are outside the thin film
  for (i = 0; i < home->newNodeKeyPtr; i++) 
    {
      nodea = home->nodeKeys[i];
      if (nodea == (Node_t *)NULL) continue;
      ...
      /* if a node is outside thin film, flag it to a surface node (6) */ 
      if (fabs(nodea->z) >= t)  
	{
	  nodea->constraint = 6;
	}
...
#if PARALLEL
      // For a segment located on two procs and ouside the TF, both nodes are set to 6
  ...
  /* 
   * First, treat the case where A is outside, B is inside the thinfilm
   */
  ...
  while (dirty6) 
      ...
      for (i = 0; i < home->newNodeKeyPtr; i++) 
	{
	  ...
	  // Node A is outside the thin film
	  if (nodea->constraint != 6) continue;
	  if (fabs(nodea->z) > t)
	    {
	      for (j = 0; j < nodea->numNbrs; j++) 
  ...
  /* 
   * Second, treat the case where A is inside, B is outside the thinfilm
   */
      ...
      for (i = 0; i < home->newNodeKeyPtr; i++) 
	{
	  ...
	  if (nodea->constraint == 0)
	      ...
	      for (j = 0; j < nodea->numNbrs; j++) 
		{
		  ...
		  // Node b is outside the thinfilm
...
//                 }
  ...
  /* 
   * Treat nodes constrained to 7 and their interactions with the thinfilm surface
   * Three cases: 
   ...
   *    - a segment with two nodes flaged 7 is outside the thinfilm
