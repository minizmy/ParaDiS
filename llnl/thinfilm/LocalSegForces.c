...
#ifdef _THINFILM
void LocalSegForces(Home_t *home, ThinFilm_t *thinfilm, int reqType)
#else
...
#ifdef _THINFILM
/*
 *                  No remote forces are calculated except if _THINFILM
 *                  is enabled. Then  we need to add in force from
 *                  the remote stress coming from the TF contribution only.
 */
                    sigb[0] = node1->sigbRem[armID12*3];
                    sigb[1] = node1->sigbRem[armID12*3+1];
                    sigb[2] = node1->sigbRem[armID12*3+2];
 ...
 */
		    ...
		    BendingForce(param, thinfilm, bx1, by1, bz1, 
				 x1, y1, z1, x2,y2, z2, f1, f2);
 ...
 */
                    ...
                    real8 LenVirtualSeg = thinfilm->LenVirtualSeg;
                    real8 dr, f[3], xs[3];

                    if (thinfilm->surfaceSegList != (real8 *)NULL)
                      {
                        VSsegList = thinfilm->surfaceSegList;

                        //SA printf("I have %d surface nodes\n",thinfilm->surfaceSegCount);

                        for (iVS = 0; iVS < thinfilm->surfaceSegCount; iVS++)
                          {
...
/*                  When _THINFILM is enabled, we need to add in
 *                  remote stress from the THINFILM contribution
