 ...
 *-------------------------------------------------------------------------*/
#ifdef _THINFILM
void CrossSlipBCC(Home_t *home, ThinFilm_t *thinfilm)
#else
...
#ifdef _THINFILM
			    SetOneNodeForce(home, thinfilm, node);
#else
...
/*
 *      Loop through all surface node and its neighbor, native to this domain
 */
        for (i = 0; i < home->newNodeKeyPtr; i++) {
	  ...
	  if (node->constraint != THINFILM_SURFACE_NODE) continue; 
 ...
 *          Only consider glide dislocations
 */
	  if (fabs(burg[X] * burg[Y] * burg[Z]) < eps) {
	    continue;
	  }

            /* Burgers vectors should be of [1 1 1] type */
	  if ( ( fabs(fabs(burg[X])-fabs(burg[Y])) > eps ) ||
	       ( fabs(fabs(burg[X])-fabs(burg[Y])) > eps ) ||
	       ( fabs(fabs(burg[X])-fabs(burg[Y])) > eps ) ) {
	    continue;
	  }

/*
 *          We must not change the glide plane for a segment
 *          that is not 'owned' by the node in the current domain.
 *          Otherwise, neighboring domains *could* change the
 *          glide plane for the same segment... this would be bad.
 */
	  if (!DomainOwnsSeg(home, opClass, thisDom, &node->nbrTag[0])) continue;

	  nbr1 = GetNeighborNode(home, node, 0);

/*
 *          Get the positions of both nodes and convert the neighbor
 *          node positions to the PBC coordinates relative to the main
 *          node.  These adjusted positions will be used during the
 *          cross slip algorithm, and if updated during the process, will
 *          be copied back into the respective nodal data structures and
 ...
 */
	  Matrix31Vector3Mult(burg, burg, tmp33);
	  
	  for (m = 0; m < 3; m++) {
	    for (n = 0; n < 3; n++) {
	      glidedir[m][n] = ((m==n) - tmp33[m][n]) * sqrt(1.5);
	    }
	  }
	  ...
	  if ((testmax2 - test2) < (4.0 * testmax2 * s2thetacrit)) 
            {
	      printf("test2 (%e, %e) node->constraint = %d\n",
		     (testmax2 - test2), (4.0 * testmax2 * s2thetacrit),
		     node->constraint );
	  ...
	  if (((testmax2 - test2) < (4.0 * testmax2 * s2thetacrit)) &&
	    ...
	    {
/*
 *              The segment are close to screw
 *
 *              Determine if the neighbor node should be considered
 *              immobile.
 */
...
#if 0 /* debug */
		printf("segment close to screw orientation pinned1 = %d constraint = %d\n",
		       pinned1,node->constraint);
#endif
