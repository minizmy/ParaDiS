 ...
 */
	if (blkFlag & LAST_BLOCK) {
		...
		double A = -2*param->tf_halfthickness/Lx;
		double B =  2*param->tf_halfthickness/Lx;
		
		drawsline(-a,-b, B,-a, b, B,colors[5],10*line_width,0);
		drawsline( a,-b, B, a, b, B,colors[5],10*line_width,0);
		drawsline(-a,-b, B, a,-b, B,colors[5],10*line_width,0);
		drawsline(-a, b, B, a, b, B,colors[5],10*line_width,0);

		drawsline(-a, b,A,-a,-b,A,colors[5],10*line_width,0);
		drawsline( a, b,A, a,-b,A,colors[5],10*line_width,0);
		drawsline(-a,-b,A, a,-b,A,colors[5],10*line_width,0);
		drawsline(-a, b,A, a, b,A,colors[5],10*line_width,0);

		drawsline(-a, 0, B, a, 0, B,colors[5],10*line_width,0);
		drawsline(-a, b/2, B, a, b/2, B,colors[5],10*line_width,0);
		drawsline(-a,-b/2, B, a,-b/2, B,colors[5],10*line_width,0);

		drawsline(0, b, B, 0, -b, B,colors[5],10*line_width,0);
		drawsline(-a/2, b, B,-a/2, -b, B,colors[5],10*line_width,0);
		drawsline(-a/2, b, B,-a/2, -b, B,colors[5],10*line_width,0);
		drawsline(a/2, b, B,a/2, -b, B,colors[5],10*line_width,0);

		drawsline(-a, 0, A, a, 0, A,colors[5],10*line_width,0);
		drawsline(-a, b/2, A, a, b/2, A,colors[5],10*line_width,0);
		drawsline(-a,-b/2, A, a,-b/2, A,colors[5],10*line_width,0);

		drawsline(0, b, A, 0, -b, A,colors[5],10*line_width,0);
		drawsline(-a/2, b, A,-a/2, -b, A,colors[5],10*line_width,0);
		drawsline(-a/2, b, A,-a/2, -b, A,colors[5],10*line_width,0);
		drawsline(a/2, b, A,a/2, -b, A,colors[5],10*line_width,0);
