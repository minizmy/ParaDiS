...
#ifdef _THINFILM
void LineTensionForce(Home_t *home, ThinFilm_t *thinfilm,
 ...
 */
	...
	BendingForce(param, thinfilm, bx, by, bz, 
 ...
 *-----------------------------------------------------------------------*/
#ifdef _THINFILM
void NodeForce(Home_t *home, ThinFilm_t *thinfilm, int reqType)
#else
 ...
 */
        if ((param->numDLBCycles > 0) && (reqType == FULL)) {
            ...
            if (elasticity) {
#ifdef _THINFILM
	        LocalSegForces(home, thinfilm, reqType);
#else
...
#ifdef _THINFILM
                    LineTensionForce(home, thinfilm,
 ...
 *
 *      When _THINFILM is enabled, then we also need 
 *      to add the TF image stress to each segment's sigbRem
 *      even to full N2 forces calculations, and even when FMM is on.
 *
 *      Exception:
 *        - If full n^2 seg/seg forces are being calculated, we don't
 *          do any remote force calcs... unless the THINFILM code is enabled,
...
#ifdef _THINFILM
/* 
 *    When _THINFILM is enabled, we ALWAYS conpute the TF contribution
...
#if 0
	LocalVirtualSegForces(home, thinfilm, FULL);
	//virtual_segment_force(home,thinfilm,FULL);
#endif
#endif
#endif

#ifdef _THINFILM
        LocalSegForces(home, thinfilm, FULL);
#else
...
#ifdef _THINFILM
void SetOneNodeForce(Home_t *home, ThinFilm_t *thinfilm, Node_t *nodeA)
#else
...
#if !defined _FEM | defined _THINFILM
/*
 *          When the THINFILM is enabled and we are doing N2 forces calcs, 
 *          we need to explicitly zero sigbRem and add in remote stress from
 *          TF in later in this function.
 */
 /*
...
#ifdef _THINFILM
	    /* When THINFILM is on, we add the remote stress contribution
	     *  in all cases: FMM on, Full N2 on etc.
             * Here we are in FULL_N2
	     */

            ComputeTF1SegSigbRem(home, thinfilm, node1, node2, armID12, armID21);
 ...
 */
/*
 *          If we need to add in any THINFILM image stress, 
 *          or we're not using the fmm code,  we need to explicitly 
 *          zero out the segment sigbRem. TF contribution is added later
 *          in this function.
 ...
 */
            if (!param->elasticinteraction) {
                int i;
#ifdef _THINFILM
                LineTensionForce(home, thinfilm, node1->x, node1->y, node1->z,
...
#if !defined _FEM & !defined _THINFILM 
/*
 *              If we are not linked with FEM and THINFILM, calculate the PK force
...
#ifdef _THINFILM
/*
 *          When linked with TF, we have to add the TF image stress
