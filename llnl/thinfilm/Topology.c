 ...
 *-------------------------------------------------------------------------*/
#ifdef _THINFILM
void SplitMultiNodes(Home_t *home, ThinFilm_t *thinfilm)
#else
...
#ifdef _THINFILM
                            SetOneNodeForce(home, thinfilm, splitNode1);
                            SetOneNodeForce(home, thinfilm, splitNode2);
#else
...
#ifdef _THINFILM
                        if (newNode->constraint == THINFILM_SURFACE_NODE) {
                            origNode->x = origPos[X];
                            origNode->y = origPos[Y];
                            origNode->z = origPos[Z];
                            newNode->constraint = UNCONSTRAINED;
                            origNode->constraint = THINFILM_SURFACE_NODE;
                        }
#endif
 ...
 */
        ...
        if (node->constraint == THINFILM_SURFACE_NODE) {
            (*splitNode1)->constraint = THINFILM_SURFACE_NODE;
            (*splitNode2)->constraint = UNCONSTRAINED;
        }
#endif
        ...
        *status = MERGE_SUCCESS;
        ...
        if ((targetNode->myTag.domainID == thisDomain) &&
            ((targetNode->constraint == UNCONSTRAINED) ||
             ((targetNode->constraint == THINFILM_SURFACE_NODE) &&
              (deadNode->constraint == THINFILM_SURFACE_NODE)))) {
#else
        if ((targetNode->myTag.domainID == thisDomain) &&
            ((targetNode->constraint == UNCONSTRAINED) ||
