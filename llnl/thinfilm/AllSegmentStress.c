...
void LocalStress(Home_t *home, ThinFilm_t *thinfilm, 
 ...
 *      local segments. 
 */
void AllSegmentStress_use_cell(Home_t *home,ThinFilm_t *thinfilm,
 ...
 */
	LocalStress(home, thinfilm, LenVirtualSeg, 
...
void LocalStress2(Home_t *home, ThinFilm_t *thinfilm, 
		 real8 LenVirtualSeg, 
		 real8 x, real8 y,real8 z,
		 int cellIndex[3],real8 locStress[3][3])
{
	...
	for (; node1 != (Node_t *)NULL; node1=node1->nextInCell) {
	  for (arm = 0; arm < node1->numNbrs; arm++) {
		...
		YoffeInfStress(MU,NU, &p2x,&p2y,&p2z,p1x,p1y,p1z,
                 ...
                 */
		real8 dr;
                if ( (node1->constraint == THINFILM_SURFACE_NODE) && (node2->constraint == UNCONSTRAINED) )
                {
                    dr = sqrt(dx*dx + dy*dy + dz*dz);
                    dr =  LenVirtualSeg / dr;
                    p1x -= dx*dr;
                ...
                if ( (node2->constraint == THINFILM_SURFACE_NODE) && (node1->constraint == UNCONSTRAINED) )
                {
                    dr = sqrt(dx*dx + dy*dy + dz*dz);
                    dr =  LenVirtualSeg / dr;
                    p2x += dx*dr;
...
void AllSegmentStress_no_cell(Home_t *home,ThinFilm_t *thinfilm,
