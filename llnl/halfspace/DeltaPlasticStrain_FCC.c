 ...
 */
	       ...
	       // the plastic strain moment. It is used later in LoadCurve.c
		    real8 HSz = node->oldz + 0.5*(nbr->oldz-node->oldz);
		    if ( fabs(halfspace->Mx) > 0.0) 
		      {
			tmpDstn = dyad[1][1]*HSz/param->simVol;
			incstn[1][1] = tmpDstn;
			// delta strain
			dstn[1][1] += tmpDstn;
		    ...
		    if ( fabs(halfspace->My) > 0.0) 
		      {
			tmpDstn = dyad[0][0]*HSz/param->simVol;
			incstn[0][0] = tmpDstn;
			// delta strain
			dstn[0][0] += tmpDstn;
			// local plastic strain
