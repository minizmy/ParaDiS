 ...
 */
        if (*disMag < 1.0e-10) {                                                
           ...
           *uz = 0.0;
           ...
           Fatal("GetUnitVector:  disMag < min:\n"
 ...
 */
        if (log) {
            AddOp(home, CHANGE_ARM_BURG,
                node1->myTag.domainID,
                node1->myTag.index,
                tag2->domainID,
                tag2->index,
                -1,-1,
...
void SubtractSegForce(Home_t *home, Node_t *node1, Node_t *node2)
{
        ...
        if ((armID = GetArmID(home, node1, node2)) < 0) return;
 ...
 *      coordinates, adjust the indices for periodic cells and
 *      calculate the cell ID.
 */
        cellIndex[X] = (int)((coord[X] - minCoord[X]) / cellSize[X]);
	if (cellIndex[X] < param->iCellNatMin) cellIndex[X] = param->iCellNatMin;
	if (cellIndex[X] > param->iCellNatMax) cellIndex[X] = param->iCellNatMax;

        cellIndex[Y] = (int)((coord[Y] - minCoord[Y]) / cellSize[Y]);
	if (cellIndex[Y] < param->jCellNatMin) cellIndex[Y] = param->jCellNatMin;
	if (cellIndex[Y] > param->jCellNatMax) cellIndex[Y] = param->jCellNatMax;

        cellIndex[Z] = (int)((coord[Z] - minCoord[Z]) / cellSize[Z]);
	if (cellIndex[Z] < param->kCellNatMin) cellIndex[Z] = param->kCellNatMin;
	if (cellIndex[Z] > param->kCellNatMax) cellIndex[Z] = param->kCellNatMax;

/* compensate for periodic cells */

	cellIndex[X]++;
        cellIndex[Y]++;
        cellIndex[Z]++;

        *cellID = EncodeCellIdx(home, cellIndex[X], cellIndex[Y], cellIndex[Z]);
