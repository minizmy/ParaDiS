...
#ifdef _HALFSPACE
/*
 *      If a node is a on the HS surface it can't be moded inside the HS, 
 *      so use that node's coordinates as the collision point
 */ 
       if (node1->constraint == HALFSPACE_SURFACE_NODE) {
            *x = p1[0];
            *y = p1[1];
            *z = p1[2];
            return;
        } else if (node2->constraint == HALFSPACE_SURFACE_NODE) {
            *x = p2[0];
            *y = p2[1];
            *z = p2[2];
            return;
