...
#ifdef _HALFSPACE 
	// Add specific parameters for HALFSPACE module (Sylvie Aubry)
        BindVar(CPList, "HS_halfthickness", &param->hs_Lzinf, V_DBL, 1, VFLAG_NULL);
        param->hs_Lzinf = 0.0;

        BindVar(CPList, "HS_nx", &param->hs_nx, V_INT, 1, VFLAG_NULL);
        param->hs_nx = 21;

        BindVar(CPList, "HS_ny", &param->hs_ny, V_INT, 1, VFLAG_NULL);
        param->hs_ny = 21;

        BindVar(CPList, "HS_VSLength", &param->hs_VSlength, V_DBL, 1,
                VFLAG_NULL);
        param->hs_VSlength = 1e4;

        BindVar(CPList, "HS_Mx", &param->hs_Mx, V_DBL, 1,
                VFLAG_NULL);
        param->hs_Mx = 0.0;

        BindVar(CPList, "HS_Bend_theta", &param->hs_Bend_theta, V_DBL, 1,
                VFLAG_NULL);
        param->hs_Bend_theta = 0.0;
