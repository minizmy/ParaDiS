 ...
 *-------------------------------------------------------------------------*/
#ifdef _HALFSPACE
void SplitMultiNodes(Home_t *home, HalfSpace_t *halfspace)
#else
...
#ifdef _HALFSPACE
                            SetOneNodeForce(home, halfspace, splitNode1);
                            SetOneNodeForce(home, halfspace, splitNode2);
#else
...
#ifdef _HALFSPACE
                        if (newNode->constraint == HALFSPACE_SURFACE_NODE) {
                            origNode->x = origPos[X];
                            origNode->y = origPos[Y];
                            origNode->z = origPos[Z];
                            newNode->constraint = UNCONSTRAINED;
                            origNode->constraint = HALFSPACE_SURFACE_NODE;
                        }
#endif
 ...
 */
        ...
        if (node->constraint == HALFSPACE_SURFACE_NODE) {
            (*splitNode1)->constraint = HALFSPACE_SURFACE_NODE;
            (*splitNode2)->constraint = UNCONSTRAINED;
        }
#endif
        ...
        *status = MERGE_SUCCESS;
        ...
        if ((targetNode->myTag.domainID == thisDomain) &&
            ((targetNode->constraint == UNCONSTRAINED) ||
             ((targetNode->constraint == HALFSPACE_SURFACE_NODE) &&
              (deadNode->constraint == HALFSPACE_SURFACE_NODE)))) {
#else
        if ((targetNode->myTag.domainID == thisDomain) &&
            ((targetNode->constraint == UNCONSTRAINED) ||
