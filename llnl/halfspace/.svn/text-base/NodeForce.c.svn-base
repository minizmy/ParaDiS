...
#ifdef _HALFSPACE
void LineTensionForce(Home_t *home, HalfSpace_t *halfspace,
 ...
 */
	...
	BendingForce(param, halfspace, bx, by, bz, 
 ...
 *-----------------------------------------------------------------------*/
#ifdef _HALFSPACE
void NodeForce(Home_t *home, HalfSpace_t *halfspace, int reqType)
#else
 ...
 */
        if ((param->numDLBCycles > 0) && (reqType == FULL)) {
            ...
            if (elasticity) {
#ifdef _HALFSPACE
	        LocalSegForces(home, halfspace, reqType);
#else
 ...
 */
	ComputeHSSegSigbRem(home, halfspace, FULL);
#endif
...
#if 0
	LocalVirtualSegForces(home, halfspace, FULL);
	//virtual_segment_force(home,halfspace,FULL);
#endif
#endif
#endif

#ifdef _HALFSPACE
        LocalSegForces(home, halfspace, FULL);
#else
...
#ifdef _HALFSPACE
void SetOneNodeForce(Home_t *home, HalfSpace_t *halfspace, Node_t *nodeA)
#else
...
#if !defined _FEM | defined _HALFSPACE
/*
 *          When the HALFSPACE is enabled and we are doing N2 forces calcs, 
 *          we need to explicitly zero sigbRem and add in remote stress from
 *          HS in later in this function.
 */
 /*
...
#ifdef _HALFSPACE
	    /* When HALFSPACE is on, we add the remote stress contribution
	     *  in all cases: FMM on, Full N2 on etc.
             * Here we are in FULL_N2
	     */

            ComputeHS1SegSigbRem(home, halfspace, node1, node2, armID12, armID21);
 ...
 */
/*
 *          If we need to add in any HALFSPACE image stress, 
 *          or we're not using the fmm code,  we need to explicitly 
 *          zero out the segment sigbRem. HS contribution is added later
 *          in this function.
 ...
 */
            if (!param->elasticinteraction) {
                int i;
#ifdef _HALFSPACE
                LineTensionForce(home, halfspace, node1->x, node1->y, node1->z,
...
#if !defined _FEM & !defined _HALFSPACE 
/*
 *              If we are not linked with FEM and HALFSPACE, calculate the PK force
...
#ifdef _HALFSPACE
/*
 *          When linked with HS, we have to add the HS image stress
