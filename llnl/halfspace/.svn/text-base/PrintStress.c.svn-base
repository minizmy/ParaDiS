 ...
 **************************************************************************/
  ...
  for (i=0;i<3;i++) GridPts[i] = halfspace->Grid[i];
#endif
  ...
  printf("\nr[50]=%f %f %f\n",GridPts[0][50][50],GridPts[1][50][50],
	 GridPts[2][50][50]);
  AllSegmentStress(home,halfspace,GridPts[0][50][50],GridPts[1][50][50],
		   GridPts[2][50][50],s1loc);
  printf("\nr[51]=%f %f %f\n",GridPts[0][51][51],GridPts[1][51][51],
	 GridPts[2][51][51]);
  AllSegmentStress(home,halfspace,GridPts[0][51][51],GridPts[1][51][51],
		   GridPts[2][51][51],s1loc);
  printf("\nr[151]=%f %f %f\n",GridPts[0][151][151],GridPts[1][151][151],
	 GridPts[2][151][151]);
  AllSegmentStress(home,halfspace,GridPts[0][151][151],GridPts[1][151][151],
  ...
  for (i=0; i<nx; i++)
      ...
      for (j=0; j<ny; j++) 
	{
	  ...
	  Init3x3(s1loc);
	  AllSegmentStress(home,halfspace,r[0],r[1],r[2],s1loc);
	  ...
	  /* Yoffe stress */
	  Init3x3(Ys);
	  AllYoffeStress(home, halfspace, r[0], r[1], r[2], Ys);
	  for (ii = 0; ii < 3; ii++)
	    for (jj = 0; jj < 3; jj++)
	      {
		s2[ii][jj] += Ys[ii][jj];
...
void PrintTractions(Home_t *home,HalfSpace_t *halfspace)
{
  ...
  for (k=0;k<3;k++)   
    for (i=0; i<nx; i++) 
      for (j=0; j<ny; j++) 
	GridPts[k][i][j] = halfspace->Grid[k][i][j];
#endif
  ...
  for (i=0; i<nx; i++)
      for (j=0; j<ny; j++) 
	{
	  fprintf(fpInf,"%.15e %.15e %.15e      %.15e %.15e %.15e\n",
		  halfspace->Grid[0][i][j],halfspace->Grid[1][i][j],-halfspace->Grid[2][i][j],
...
void PrintdSegImgStress(Home_t *home, HalfSpace_t *halfspace)
{
  ...
  for (i=0; i<nx; i++)
  {
	...
	r[2] = halfspace->Grid[2][i][j]; /* Top surface */
	//r[2] =-halfspace->Grid[2][i][j]; /* Bottom surface */
