 ...
 */
		...
		ComputeHS1SegSigbRem(home, halfspace, node, 
				     nbr, armID1, armID2);
	      }
	    }
			  ...
			  int armID1, int armID2)
{
...
#ifdef _HSIMGSTRESS
      DispStress(halfspace,r,totRemSig);
#endif  
  ...
  AllYoffeStress(home, halfspace, r[0], r[1], r[2], YoffeStress);
		  ...
		  real8 f1[3], real8 f2[3])
{
  ...
  // Moment of Inertia
  II = (2*halfspace->t)*(2*halfspace->t)*(2*halfspace->t)/12.0;

  //str[0][0] = - halfspace->My * z/II;
  //str[1][1] = - halfspace->Mx * z/II;
  ...
  str11 = - halfspace->Mx * z/II;
  bend_angle = halfspace->Bend_theta;
 ...
 */
		...
		if (node1->constraint==0 && node2->constraint==0) continue;
		if (node1->constraint==7 ||  node2->constraint==7) continue;
		if (node1->constraint==6 && node2->constraint==6) continue;
 ...
 */
		...
		for (k = 0; k < totalSegs; k++) {
		    ...
		    if (node3->constraint==6 && node4->constraint==6) continue;
		    // End specific to virtual segments
 ...
 */
		    ...
		    ComputeVirtualForces(home, halfspace,
			  ...
			  real8 *f3, real8 *f4)
{
	...
	if (nodea->constraint == 6)
	  { 
	    node2 = nodea;
	    node1 = nodeb;
	  }
	else if (nodeb->constraint == 6)
	  {
	    node2 = nodeb;
	    node1 = nodea;
	  }
 ...
 * of virtual segments on the segments inside the halfspace.
 */
void virtual_segment_force(Home_t *home,HalfSpace_t *halfspace, int reqType)
{
  int i,j;
  int nnbrs1,nnbrs3;
  int ti,tj,armID12,armID21,armID34,armID43;
  Node_t  *Node1,*Node2,*Node3,*Node4;
  Param_t *param;
  real8 x1,x2,x3,x4,y1,y2,y3,y4,z1,z2,z3,z4,xm,ym,zm;
  real8 fx1=0.0,fx2=0.0,fy1=0.0,fy2=0.0,fz1=0.0,fz2=0.0;
  real8 fx3=0.0,fx4=0.0,fy3=0.0,fy4=0.0,fz3=0.0,fz4=0.0;
  ...
  real8 Lx,Ly,LenVirtualSeg;
  int seg12Local,nbrIsLocal;
  ...
  for (i = 0; i < home->newNodeKeyPtr; i++) 
    {
      ...
      if (Node1->constraint!=6) continue; 
      // Node 1 is on the surface : flag 6
      ...
      for (ti=0;ti<nnbrs1;ti++) 
	{
	  ...
	  if (Node2->constraint==6) continue; // Node 2 inside
	  // Node 2 flag 0 : is not on the surface	    
	  ...
	  // node 1 is virtual
	  // node 2 is on the surface now where node 1 was

	  armID12 = ti;//GetArmID(home, Node1, Node2);
	  armID21 = GetArmID(home, Node2, Node1);
	  ...
	  for (j = 0; j < home->newNodeKeyPtr; j++) 
	    {
	      ...
	      // Loop over all neighbor nodes of Node 3
	      ...
	      for (tj=0;tj<nnbrs3;tj++) 
		{
		  ...
		  // Node 3 and 4 are not on the surface together
		  if (Node3->constraint==6 && Node4->constraint==6) continue;
	    
		  if (OrderNodes(Node3, Node4) >= 0) continue;
		  //if (NodeOwnsSeg(home, Node3, Node4) == 0) continue;
		  ...
		  if (setSeg34Forces)
		    {
		      armID34 = tj;//GetArmID(home, Node3, Node4);
		      armID43 = GetArmID(home, Node4, Node3);
		      ...
		      printf("f3 = %f %f %f\n",
			     R[0][0]*fx3+R[0][1]*fy3+R[0][2]*fz3,
			     R[1][0]*fx3+R[1][1]*fy3+R[1][2]*fz3,
			     R[2][0]*fx3+R[2][1]*fy3+R[2][2]*fz3);
		      printf("f4 = %f %f %f\n\n",
			     R[0][0]*fx4+R[0][1]*fy4+R[0][2]*fz4,
			     R[1][0]*fx4+R[1][1]*fy4+R[1][2]*fz4,
			     R[2][0]*fx4+R[2][1]*fy4+R[2][2]*fz4);
		      ...
		      SemiInfiniteSegSegForce(x2, y2, z2, x1, y1, z1, 
					      x3, y3, z3, x4, y4, z4,
					      -b12x, -b12y, -b12z, b34x, b34y, b34z,        
					      a, MU, NU, seg12Local, seg34Local,
					      &fx1, &fy1, &fz1, &fx2, &fy2, &fz2,
		      ...
		      SemiInfiniteSegSegForce2(x2, y2, z2, x1, y1, z1, 
					      x3, y3, z3, x4, y4, z4,
					      -b12x,-b12y,-b12z,b34x,b34y,b34z,   
					      a, MU, NU, seg12Local, seg34Local,
					      &fx1, &fy1, &fz1, 
