...
#ifdef _HALFSPACE
void LocalSegForces(Home_t *home, HalfSpace_t *halfspace, int reqType)
#else
...
#ifdef _HALFSPACE
/*
 *                  No remote forces are calculated except if _HALFSPACE
 *                  is enabled. Then  we need to add in force from
 *                  the remote stress coming from the HS contribution only.
 */
                    sigb[0] = node1->sigbRem[armID12*3];
                    sigb[1] = node1->sigbRem[armID12*3+1];
                    sigb[2] = node1->sigbRem[armID12*3+2];
 ...
 */
		    ...
		    BendingForce(param, halfspace, bx1, by1, bz1, 
				 x1, y1, z1, x2,y2, z2, f1, f2);
 ...
 */
                    ...
                    real8 LenVirtualSeg = halfspace->LenVirtualSeg;
                    real8 dr, f[3], xs[3];

                    if (halfspace->surfaceSegList != (real8 *)NULL)
                      {
                        VSsegList = halfspace->surfaceSegList;

                        //SA printf("I have %d surface nodes\n",halfspace->surfaceSegCount);

                        for (iVS = 0; iVS < halfspace->surfaceSegCount; iVS++)
                          {
...
/*                  When _HALFSPACE is enabled, we need to add in
 *                  remote stress from the HALFSPACE contribution
