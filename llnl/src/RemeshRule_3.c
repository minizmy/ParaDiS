 ...
 *                      version 3 remesh for coarsening or refining
 *                      the mesh topology.  This version is identical to
...
static void MeshRefine(Home_t *home)
{
        ...
        real8   areaMax, areaMax2, minSeg2, maxSeg2;
        real8   delta, r1, r2, r3, s, area2;
        real8   dr1dt, dr2dt, dr3dt, dsdt, darea2dt;
        ...
        Node_t  *node, *nbr1, *nbr2, *splitNode1, *splitNode2;
        Param_t *param;
        real8   p1, p2, p3;
        real8   radius, np, norm, tmp, eps = 1.0e-12;
        real8   tmp3[3], center[3], dir1[3], dir2[3], nplane[3], diff[3];
        real8   mid1[3], mid2[3], mv3[3], rhs[3], burg[3];
 ...
 *              and its two neighbors is increasing or decreasing.  However,
 *              only calculate the effects of the velocity of the primary
 *              node on that area (i.e. treat the neighboring nodes as
 *              if they had zero velocity).  If we don't do this, there
 *              are situations where although the area is increasing
 *              the primary node is moving *toward* the other two nodes
 *              and placing new nodes on an arc containing the 3 nodes
 *              turns out to be the incorrect placement.
 */
...
/*
 *              Check each arm to determine if it should be
 *              bissected.  There are some restrictions;
 *
 *              1) Segments will only be bissected by the owning
 *                 domain 
 *              2) Local segments will only be bissected when the
 *                 neighbor node has a higher tag index than the
 *                 current node's.
 *              3) If the area in the triangle defined by the
 *                 three nodes is decreasing, don't refine either
 *                 segment.
 */
        
/*
 *              First check if the second arm needs to be split.
 */
                splitOK = DomainOwnsSeg(home, OPCLASS_REMESH,
 ...
 *                  to the node being created... in this case
 *                  arm at index one.
 */
                    *armList = 1;
                    ...
                    if (area2 > areaMax2) {
                        ...
                        if (Matrix33Invert(mat, invMat) == 0) {
                            Fatal("MeshRefine: Cannot invert 3X3 matrix!");
                        }
                        ...
                        tmp3[X] = mid2[X] - center[X];
                        tmp3[Y] = mid2[Y] - center[Y];
                        tmp3[Z] = mid2[Z] - center[Z];
                        ...
                        diff[X] = (radius*dir2[X]) - (mid2[X]-center[X]);
                        diff[Y] = (radius*dir2[Y]) - (mid2[Y]-center[Y]);
                        diff[Z] = (radius*dir2[Z]) - (mid2[Z]-center[Z]);
                        ...
                        newPos[X] = mid2[X] + diff[X];
                        newPos[Y] = mid2[Y] + diff[Y];
                        newPos[Z] = mid2[Z] + diff[Z];

                    } else {
                        newPos[X] = node->x + (vec2[X] * 0.5);
                        newPos[Y] = node->y + (vec2[Y] * 0.5);
                        newPos[Z] = node->z + (vec2[Z] * 0.5);
                    }

                    newVel[X] = (node->vX + nbr2->vX) * 0.5;
                    newVel[Y] = (node->vY + nbr2->vY) * 0.5;
                    newVel[Z] = (node->vZ + nbr2->vZ) * 0.5;
        
                    EstRefinementForces(home, node, nbr2, newPos, vec2,
 ...
 */
                    globalOp = (nbr2->myTag.domainID != node->myTag.domainID);
...
/*
 *              Check if the first arm should be bissected
 */
                splitOK = DomainOwnsSeg(home, OPCLASS_REMESH,
...
#if 1
/*
 *                  If we refined the other segment attached to this
 *                  node, only refine this one if it exceeds maxSeg.
 *                  (If we refined the other segment, the area criteria
 *                  is not necessarily valid anymore, so we won't use
 *                  it again this cycle.  If area is still too large,
 *                  the segment will likely be refined next cycle.
 */
                    if (splitSeg2 && (r1 < param->maxSeg)) {
                        continue;
                    }
#endif

/*
 *                  When bisecting a segment, we always move exactly one
 *                  arm from the original node to the node being
 *                  created... in this case arm at index zero.
 */
                    *armList = 0;
        
                    if (area2 > areaMax2) {

                        Matrix33Invert(mat, invMat);
                        Matrix33Vector3Multiply(invMat, rhs, center);
                        ...
                        radius = Normal(tmp3);
                        tmp3[X] = mid1[X] - center[X];
                        tmp3[Y] = mid1[Y] - center[Y];
                        tmp3[Z] = mid1[Z] - center[Z];
                        ...
                        diff[X] = (radius*dir1[X]) - (mid1[X]-center[X]);
                        diff[Y] = (radius*dir1[Y]) - (mid1[Y]-center[Y]);
                        diff[Z] = (radius*dir1[Z]) - (mid1[Z]-center[Z]);
                        ...
                        newPos[X] = mid1[X] + diff[X];
                        newPos[Y] = mid1[Y] + diff[Y];
                        newPos[Z] = mid1[Z] + diff[Z];
