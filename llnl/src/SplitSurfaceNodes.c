 ...
 */
        if (origNode->constraint != SURFACE_NODE) {

            origNode->x = origPos[X];
            origNode->y = origPos[Y];
            origNode->z = origPos[Z];

            origNode->constraint = SURFACE_NODE;

            origNode->fem_Surface[0] = newNode->fem_Surface[0];
            origNode->fem_Surface[1] = newNode->fem_Surface[1];

            origNode->fem_Surface_Norm[0] = newNode->fem_Surface_Norm[0];
            origNode->fem_Surface_Norm[1] = newNode->fem_Surface_Norm[1];
            origNode->fem_Surface_Norm[2] = newNode->fem_Surface_Norm[2];
        }
 ...
 */
        for (i = 0; i < home->newNodeKeyPtr; i++) {
            ...
            if ((node->numNbrs < 2) || (node->constraint != SURFACE_NODE)) {
                continue;
            }
