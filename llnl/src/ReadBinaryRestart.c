...
static Node_t *FindInNode(Node_t *nodeList, int numNodes, Tag_t *tag)
{
        ...
        found = bsearch(&tmpNode, nodeList, numNodes, sizeof(Node_t),
                        OrderNodes);
 ...
 */
        datasetID = H5Dopen(fileID, datasetName);
 ...
 */
                    ...
                    if (neededCnt > nodeArrayCnt) {
                        if (readCount > 0) {
                            ...
                            inData->node = realloc(inData->node,
 ...
 *          the burgers vector is conserved for all unconstrained nodes
 */
